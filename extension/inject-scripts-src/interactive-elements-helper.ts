...
(function () {
  ...
  function findInteractiveElements(options = {}) {
    const {
      ...
      types = Object.keys(ELEMENT_CONFIG),
    } = options;
  ...
  function findElementsByTextWithFallback(options = {}) {
    ...
    let results = findInteractiveElements({
      ...options,
      types: Object.keys(ELEMENT_CONFIG),
    });
    if (results.length > 0) {
    ...
    if (textNodes.snapshotLength > 0) {
      for (let i = 0; i < textNodes.snapshotLength; i++) {
        ...
        if (parentElement) {
          const interactiveAncestor = parentElement.closest(
            ANY_INTERACTIVE_SELECTOR,
          );
          if (
            interactiveAncestor &&
    ...
    const finalElements = Array.from(leafElements).filter((el) => {
      return ![...leafElements].some(
        (otherEl) => el !== otherEl && el.contains(otherEl),
      );
    });
