if (window.__WEB_FETCHER_HELPER_INITIALIZED__) {
  ...
  chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
    ...
    if (request.action === 'chrome_web_fetcher_ping') {
      ...
      try {
        ...
        if (request.selector) {
          ...
          if (element) {
            ...
            throw new Error(
              `No element found matching selector: ${request.selector}`,
            );
          }
        } else {
      ...
      try {
        ...
        if (request.selector) {
          ...
          if (article && article.textContent) {
            ...
            if (
              iframeContent &&
              iframeContent.trim().length > config.minTextLength
            ) {
              fullContent += '\n\n--- Embedded Content ---\n\n' + iframeContent;
            }
  ...
  function extractPageMetadata() {
    ...
    if (publishedElement) {
      metadata.published =
        publishedElement.getAttribute('content') ||
        publishedElement.getAttribute('datetime') ||
        '';
    }

    // Extract site name
    const siteNameElement = document.querySelector(
      ...
      try {
        if (isSameOrigin(iframe) && isElementVisible(iframe)) {
          ...
          if (doc) {
            ...
            if (
              iframeText &&
              iframeText.trim().length >= config.minTextLength
            ) {
