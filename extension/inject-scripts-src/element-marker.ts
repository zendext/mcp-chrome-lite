(function () {
  ...
  const PanelHost = (() => {
    ...
    function mount() {
      ...
      hostElement.querySelector = (...args) =>
        shadowRoot.querySelector(...args);
      hostElement.querySelectorAll = (...args) =>
        shadowRoot.querySelectorAll(...args);
  ...
  const StateStore = (() => {
    ...
    function updateTabUI() {
      ...
      if (attrContent)
        attrContent.style.display =
          state.activeTab === 'attributes' ? 'block' : 'none';
      if (executeContent)
        executeContent.style.display =
          state.activeTab === 'execute' ? 'block' : 'none';
  ...
  function drawRectBoxes(
    ...
    for (let i = 0; i < count; i++) {
      ...
      const x = Number.isFinite(r.left)
        ? r.left
        : Number.isFinite(r.x)
          ? r.x
          : 0;
  ...
  function getDeepPageTarget(ev) {
    ...
    try {
      const path =
        typeof ev.composedPath === 'function' ? ev.composedPath() : null;
  ...
  function processMouseMove(ev) {
    ...
    if (!IS_MAIN) {
      try {
        const list = listMode
          ? computeElementList(target) || [target]
          : [target];
  ...
  async function verifyHighlightOnly() {
    try {
      const selector = STATE.box
        ?.querySelector('#__em_selector')
        ?.textContent?.trim();
      if (!selector) return;
  ...
  async function verifySelectorNow() {
    try {
      ...
      if (action === 'type_text') {
        ...
        if (!actionText) {
          StateStore.set({
            validation: {
              status: 'failure',
              message: 'Text is required for type_text',
            },
          });
          return;
        }
      ...
      if (action === 'press_keys') {
        ...
        if (!actionKeys) {
          StateStore.set({
            validation: {
              status: 'failure',
              message: 'Keys are required for press_keys',
            },
          });
          return;
        }
      ...
      if (action === 'scroll') {
        const direction =
          STATE.box?.querySelector('#__em_scroll_direction')?.value || 'down';
        const rawAmount = Number(
          STATE.box?.querySelector('#__em_scroll_distance')?.value,
        );
      ...
      if (['left_click', 'double_click', 'right_click'].includes(action)) {
        ...
        payload.waitForNavigation =
          !!STATE.box?.querySelector('#__em_wait_nav')?.checked;
        payload.timeoutMs =
          Number(STATE.box?.querySelector('#__em_nav_timeout')?.value) || 3000;
      }
      ...
      const history = [
        ...(StateStore.get('validationHistory') || []),
        newEntry,
      ].slice(-5);
   ...
   */
  async function highlightSelectorExternal({
    selector,
    selectorType = 'css',
    listMode = false,
  }) {
    ...
    try {
      ...
      if (normalized.includes('|>')) {
        ...
        if (parts.length >= 2) {
          ...
          if (
            ...
            return {
              success: false,
              error: `Frame element not found: ${frameSel}`,
            };
          }
          ...
          if (!cw) {
            return {
              success: false,
              error: 'Unable to access frame contentWindow',
            };
          }
      ...
      const matches =
        effectiveType === 'xpath'
          ? evaluateXPathAll(normalized)
          : queryAllDeep(normalized);
  ...
  function bindControls() {
    ...
    host
      .querySelector('#__em_verify')
      ?.addEventListener('click', verifyHighlightOnly);
    host
      .querySelector('#__em_execute')
      ?.addEventListener('click', verifySelectorNow);

    // Copy
    host
      .querySelector('#__em_copy')
      ?.addEventListener('click', copySelectorNow);
    host
      .querySelector('#__em_copy_selector')
      ?.addEventListener('click', copySelectorNow);
    ...
    // Selector type
    host
      .querySelector('#__em_selector_type')
    ...
    host.querySelector('#__em_toggle_tab')?.addEventListener('click', () => {
      ...
      StateStore.set({
        activeTab: currentTab === 'attributes' ? 'execute' : 'attributes',
      });
    });
    ...
    host.querySelector('#__em_pref_id')?.addEventListener('change', (e) => {
      const prefs = {
        ...StateStore.get('prefs'),
        preferId: !!e.target.checked,
      };
    ...
    host.querySelector('#__em_pref_attr')?.addEventListener('change', (e) => {
      const prefs = {
        ...StateStore.get('prefs'),
        preferStableAttr: !!e.target.checked,
      };
      StateStore.set({ prefs });
    });
    ...
    (ev) => {
      try {
        ...
        if (data.type === 'em_hover' && Array.isArray(data.rects)) {
          ...
          const composite = frameSel
            ? `${frameSel} |> ${data.innerSel}`
            : data.innerSel;
          const selectorText = STATE.box?.querySelector('#__em_selector');
          const selectorDisplay = STATE.box?.querySelector(
            '#__em_selector_text',
          );
