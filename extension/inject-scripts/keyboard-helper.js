...
if (window.__KEYBOARD_HELPER_INITIALIZED__) {
  ...
  function parseSingleKeyCombination(keyString) {
    ...
    for (const part of parts) {
      if (MODIFIER_KEYS[part]) {
        ...
        console.error(
          `Invalid key combination string: ${keyString}. Multiple main keys found.`,
        );
        return null;
      }
    }
    ...
    if (!mainKeyPart) {
      ...
      if (
        Object.keys(MODIFIER_KEYS).includes(parts[parts.length - 1]) &&
        parts.length === 1
      ) {
        ...
        if (mainKeyPart === 'alt')
          return { key: 'Alt', code: 'AltLeft', keyCode: 18, modifiers };
        ...
        console.error(
          `Invalid key combination string: ${keyString}. No main key specified.`,
        );
        return null;
      }
    }
  ...
  function dispatchKeyEvents(parsedKeyInfo, element) {
    if (!parsedKeyInfo)
      return {
        success: false,
        error: 'Invalid key info provided for dispatch.',
      };
    ...
    try {
      const kdRes = element.dispatchEvent(
    ...
    if (request.action === 'simulateKeyboard') {
      ...
      simulateKeyboard(request.keys, targetEl, request.delay)
        ...
        .catch((error) => {
          ...
          console.error(
            'Unexpected error in simulateKeyboard promise chain:',
            error,
          );
          sendResponse({
            success: false,
