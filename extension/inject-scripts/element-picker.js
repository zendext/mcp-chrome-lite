...
(function () {
  ...
  function summarizeText(el) {
    ...
    try {
      ...
      if (placeholder && placeholder.trim())
        return placeholder.trim().slice(0, MAX_TEXT_LEN);
  ...
  function startSession(payload) {
    const sessionId =
      payload && payload.sessionId ? String(payload.sessionId) : '';
    if (!sessionId) return;

    STATE.active = true;
    STATE.sessionId = sessionId;
    STATE.activeRequestId =
      payload && payload.activeRequestId
        ? String(payload.activeRequestId)
        : null;
    ensureHighlighter();
    attachListeners();
  }
