...
class NetworkRequestTool extends BaseBrowserToolExecutor {
  ...
  async execute(args: NetworkRequestToolParams): Promise<ToolResult> {
    ...
    try {
      ...
      await this.injectContentScript(activeTabId, [
        'inject-scripts/network-helper.js',
      ]);
      ...
      console.log(
        `NetworkRequestTool: Response from content script:`,
        resultFromContentScript,
      );
      ...
      console.error(
        'NetworkRequestTool: Error sending network request:',
        error,
      );
