...
function getOrCreateStopPromise(
...
class PerformanceStartTraceTool extends BaseBrowserToolExecutor {
  ...
  async execute(args: StartTraceParams): Promise<ToolResult> {
    ...
    try {
      ...
      if (existed?.recording) {
        return {
          content: [
            {
              type: 'text',
              text: 'Error: a performance trace is already running.',
            },
          ],
          isError: false,
        };
      ...
      if (reload) {
        try {
          await cdpSessionManager.sendCommand(tabId, 'Page.reload', {
            ignoreCache: true,
          });
        } catch {
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              success: true,
              message:
                'Performance trace is recording. Use performance_stop_trace to stop it.',
              reload,
              autoStop,
            }),
          },
      ...
      return createErrorResponse(
        `Failed to start performance trace: ${e?.message || e}`,
      );
    }
  }
}
...
class PerformanceStopTraceTool extends BaseBrowserToolExecutor {
  ...
  async execute(args: StopTraceParams): Promise<ToolResult> {
    ...
    try {
      ...
      if (!session) {
        return {
          content: [
            {
              type: 'text',
              text: 'No performance trace session found for the current tab.',
            },
          ],
          isError: false,
        };
      ...
      let saved:
        | { downloadId?: number; filename?: string; fullPath?: string }
        | undefined;
      if (saveToDownloads) {
        saved = await saveTraceToDownloads(
          json,
          filenamePrefix || 'performance_trace',
        );
      } else {
      ...
      return createErrorResponse(
        `Failed to stop performance trace: ${e?.message || e}`,
      );
    }
  }
}
...
class PerformanceAnalyzeInsightTool extends BaseBrowserToolExecutor {
  ...
  async execute(
    args: AnalyzeInsightParams & { timeoutMs?: number },
  ): Promise<ToolResult> {
    ...
    try {
      ...
      const fullPath =
        (result.saved && (result.saved as any).fullPath) || undefined;
      if (fullPath) {
        try {
          const requestId = `trace-analyze-${Date.now()}-${Math.random().toString(36).slice(2)}`;
          const timeoutMs = Math.max(
            10000,
            Math.min((args as any)?.timeoutMs ?? 60000, 300000),
          );
          const resp = await new Promise<any>((resolve, reject) => {
            const timer = setTimeout(() => {
              chrome.runtime.onMessage.removeListener(listener);
      ...
      for (const ev of result.events.slice(0, 100000)) {
        const n =
          typeof (ev as any)?.name === 'string' ? (ev as any).name : 'unknown';
...
export const performanceAnalyzeInsightTool =
  new PerformanceAnalyzeInsightTool();
