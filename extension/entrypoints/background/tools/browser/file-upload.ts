  ...
  async execute(args: FileUploadToolParams): Promise<ToolResult> {
    ...
    try {
      ...
      await cdpSessionManager.withSession(tabId, 'file-upload', async () => {
        ...
        // Get the document
        const { root } = (await cdpSessionManager.sendCommand(
          tabId,
          'DOM.getDocument',
          {
            depth: -1,
        ...
        const { nodeId } = (await cdpSessionManager.sendCommand(
          tabId,
          'DOM.querySelector',
          {
            nodeId: root.nodeId,
        ...
        const { node } = (await cdpSessionManager.sendCommand(
          tabId,
          'DOM.describeNode',
          {
            nodeId,
        ...
        if (node.nodeName !== 'INPUT') {
          throw new Error(
            `Element with selector "${selector}" is not an input element`,
          );
        }
