...
class NetworkCaptureStartTool extends BaseBrowserToolExecutor {
  ...
  private handleTabRemoved(tabId: number) {
    if (this.captureData.has(tabId)) {
      console.log(
        `NetworkCaptureV2: Tab ${tabId} was closed, cleaning up resources.`,
      );
  ...
  private async handleTabCreated(tab: chrome.tabs.Tab) {
    try {
      ...
      console.log(
        `NetworkCaptureV2: Successfully extended capture to new tab ${newTabId}`,
      );
    } catch (error) {
      console.error(
        `NetworkCaptureV2: Error extending capture to new tab:`,
        error,
      );
    }
  }
  ...
  private shouldFilterRequest(url: string, includeStatic: boolean): boolean {
    ...
    if (
      AD_ANALYTICS_DOMAINS.some((pattern) => normalizedUrl.includes(pattern))
    ) {
      return true;
    }
  ...
  private shouldFilterByMimeType(
    ...
    if (
      NetworkCaptureStartTool.API_MIME_TYPES.some((type) =>
    ...
    if (!includeStatic) {
      ...
      if (
        ...
        console.log(
          `NetworkCaptureV2: Filtering static resource by MIME type: ${mimeType}`,
        );
        return true;
      }
  ...
  private updateLastActivityTime(tabId: number): void {
    ...
    if (captureInfo.inactivityTimeout > 0) {
      this.inactivityTimers.set(
        tabId,
        setTimeout(
          () => this.checkInactivity(tabId),
          captureInfo.inactivityTimeout,
        ),
      );
    }
  }
  ...
  private checkInactivity(tabId: number): void {
    ...
    const lastActivity =
      this.lastActivityTime.get(tabId) || captureInfo.startTime;
    const now = Date.now();
  ...
  private async stopCaptureByInactivity(tabId: number): Promise<void> {
    ...
    console.log(
      `NetworkCaptureV2: Stopping capture due to inactivity for tab ${tabId}`,
    );
  ...
  private setupListeners(): void {
    ...
    if (
      Object.prototype.hasOwnProperty.call(this.listeners, 'onBeforeRequest')
    ) {
      return;
    }
    ...
    this.listeners.onCompleted = (
      ...
      if ('responseSize' in details) {
        requestInfo.responseSize = details.fromCache
          ? 0
          : (details as any).responseSize;
      }
  ...
  private removeListeners(): void {
    ...
    console.log(
      `NetworkCaptureV2: No more active captures, removing all listeners.`,
    );
  ...
  private async startCaptureForTab(
    ...
    try {
      ...
      console.error(
        `NetworkCaptureV2: Error starting capture for tab ${tabId}:`,
        error,
      );
  ...
  public async stopCapture(
    ...
    if (!captureInfo) {
      ...
      return {
        success: false,
        message: `No capture in progress for tab ${tabId}`,
      };
    }
    ...
    try {
      ...
      const commonRequestHeaders = this.analyzeCommonHeaders(
        requestsArray,
        'requestHeaders',
      );
      const commonResponseHeaders = this.analyzeCommonHeaders(
        requestsArray,
        'responseHeaders',
      );
      ...
      console.error(
        `NetworkCaptureV2: Error stopping capture for tab ${tabId}:`,
        error,
      );
  ...
  private analyzeCommonHeaders(
    ...
    const headers = firstRequestWithHeaders[headerType] as Record<
      string,
      string
    >;
    ...
    try {
      ...
      if (targetUrl) {
        ...
        if (matchingTabs.length > 0) {
          ...
          console.log(
            `NetworkCaptureV2: Found existing tab with URL: ${targetUrl}`,
          );
        } else {
          // Create new tab
          console.log(
            `NetworkCaptureV2: Creating new tab with URL: ${targetUrl}`,
          );
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              success: true,
              message:
                'Network capture V2 started successfully, waiting for stop command.',
...
class NetworkCaptureStopTool extends BaseBrowserToolExecutor {
  ...
  async execute(): Promise<ToolResult> {
    ...
    try {
      ...
      if (!startTool) {
        return createErrorResponse(
          'Network capture V2 start tool instance not found',
        );
      }
      ...
      if (ongoingCaptures.length === 0) {
        return createErrorResponse(
          'No active network captures found in any tab.',
        );
      }
      ...
      if (!stopResult.success) {
        return createErrorResponse(
          stopResult.message ||
            `Failed to stop network capture for tab ${primaryTabId}`,
        );
      }
      ...
      if (ongoingCaptures.length > 1) {
        ...
        for (const tabId of otherTabIds) {
          try {
            ...
            console.error(
              `NetworkCaptureStopTool: Error stopping capture on tab ${tabId}:`,
              error,
            );
          }
        }
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              ...
              commonResponseHeaders:
                stopResult.data?.commonResponseHeaders || {},
              ...
              totalRequestsReceived:
                stopResult.data?.totalRequestsReceived || 0,
              requestLimitReached:
                stopResult.data?.requestLimitReached || false,
