...
} from './network-capture-debugger';
import {
  networkCaptureStartTool,
  networkCaptureStopTool,
} from './network-capture-web-request';
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
      return {
        ...result,
        content: [
          { type: 'text', text: JSON.stringify({ ...parsed, ...extra }) },
        ],
      };
    }
  } catch {
  ...
  async execute(args: NetworkCaptureToolParams): Promise<ToolResult> {
    ...
    if (action !== 'start' && action !== 'stop') {
      return createErrorResponse(
        'Parameter [action] is required and must be one of: start, stop',
      );
    }
