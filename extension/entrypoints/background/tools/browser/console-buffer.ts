...
function formatConsoleArgs(args: unknown[]): string {
  ...
  return args
    .map((arg: unknown) => {
      ...
      if (a.type === 'function')
        return (a.description as string) || '[Function]';
...
function extractArgPreview(arg: unknown): unknown {
  ...
  if ('unserializableValue' in a)
    preview.unserializableValue = a.unserializableValue;
...
class ConsoleBuffer {
  ...
  private handleDebuggerEvent(
    ...
    if (method === 'Runtime.consoleAPICalled' && p) {
      ...
      const callFrame = stackTrace?.callFrames?.[0] as
