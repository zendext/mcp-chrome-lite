...
if (typeof __MAX_CAP_RATE === 'number' && __MAX_CAP_RATE > 0) {
  ...
  const requiredExtraDelay = Math.max(
    0,
    minIntervalMs - SCREENSHOT_CONSTANTS.SCROLL_DELAY_MS,
  );
...
class ScreenshotTool extends BaseBrowserToolExecutor {
    ...
    try {
      ...
      if (canUseCdpCapture) {
        try {
          ...
          console.warn(
            'CDP viewport capture failed, falling back to helper path:',
            e,
          );
        }
      }
      ...
      if (!finalImageDataUrl) {
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, SCREENSHOT_CONSTANTS.SCRIPT_INIT_DELAY),
        );
        ...
        if (fullPage) {
          ...
          finalImageDataUrl = await this._captureFullPage(
            tab.id!,
            args,
            pageDetails,
          );
          ...
          finalImageDataUrl = await chrome.tabs.captureVisibleTab(
            tab.windowId,
            { format: 'png' },
          );
          finalImageWidthCss = pageDetails.viewportWidth;
      ...
      if (storeBase64 === true) {
        ...
        const base64Data = compressed.dataUrl.replace(
          /^data:image\/[^;]+;base64,/,
          '',
        );
  ...
  async _captureFullPage(
    ...
    const maxHeightPx =
      options.maxHeight || SCREENSHOT_CONSTANTS.MAX_CAPTURE_HEIGHT_PX;
    ...
    while (
      capturedHeightPx < totalHeightPx &&
      partIndex < SCREENSHOT_CONSTANTS.MAX_CAPTURE_PARTS
    ) {
      this.logInfo(
      ...
      if (!dataUrl)
        throw new Error(
          'captureVisibleTab returned empty during full page capture',
        );
      ...
      const lastPartEffectiveHeightPx = Math.min(
        imgForHeight.height,
        totalHeightPx - yOffsetPx,
      );
      ...
      if (
        capturedHeightPx >=
        totalHeightPx - SCREENSHOT_CONSTANTS.PIXEL_TOLERANCE
      )
        break;
