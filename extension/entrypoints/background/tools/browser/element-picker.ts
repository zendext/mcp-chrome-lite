  ...
  async execute(args: ElementPickerToolParams): Promise<ToolResult> {
    ...
    if (rawRequests.length === 0) {
      return createErrorResponse(
        `${ERROR_MESSAGES.INVALID_PARAMETERS}: requests[] is required`,
      );
    }
    ...
    const onRuntimeMessage = (
      ...
      const msg = message as
        | Partial<PickerUiEvent & PickerFrameEvent>
        | undefined;
      ...
      if (msg.type === BACKGROUND_MESSAGE_TYPES.ELEMENT_PICKER_FRAME_EVENT) {
        ...
        if (msg.event === 'selected') {
          ...
          // Validate request ID
          const reqExists =
            requestId && requests.some((r) => r.id === requestId);
          if (!reqExists) {
          ...
          const raw = (msg.element || {}) as Partial<
            Omit<PickedElement, 'frameId'>
          >;
          ...
          const picked: PickedElement = {
            ...
            center:
              center && typeof center === 'object' ? center : { x: 0, y: 0 },
    ...
    try {
      ...
      await this.callPickerApi(tabId, 'startSession', {
        sessionId,
        activeRequestId,
      });
