...
class NetworkDebuggerStartTool extends BaseBrowserToolExecutor {
  ...
  private handleTabRemoved(tabId: number) {
    if (this.captureData.has(tabId)) {
      console.log(
        `NetworkDebuggerStartTool: Tab ${tabId} was closed, cleaning up resources.`,
      );
  ...
  private async handleTabCreated(tab: chrome.tabs.Tab) {
    try {
      ...
      console.log(
        `NetworkDebuggerStartTool: Successfully extended capture to new tab ${newTabId}`,
      );
    } catch (error) {
      console.error(
        `NetworkDebuggerStartTool: Error extending capture to new tab:`,
        error,
      );
    }
  }
    ...
    try {
      ...
      try {
        ...
        await cdpSessionManager
          ...
          .catch((e) =>
            console.warn('Error detaching after failed enable:', e),
          );
        throw error;
      }
      ...
      console.error(
        `NetworkDebuggerStartTool: Error starting capture for tab ${tabId}:`,
        error,
      );
  ...
  private checkInactivity(tabId: number) {
    ...
    const lastActivity =
      this.lastActivityTime.get(tabId) || captureInfo.startTime; // Use startTime if no activity yet
    const now = Date.now();
    ...
    if (inactiveTime >= captureInfo.inactivityTimeout) {
      ...
      const remainingTime = Math.max(
        0,
        captureInfo.inactivityTimeout - inactiveTime,
      );
      this.inactivityTimers.set(
        tabId,
  ...
  private async stopCaptureByInactivity(tabId: number) {
    ...
    console.log(
      `NetworkDebuggerStartTool: Stopping capture due to inactivity for tab ${tabId}.`,
    );
  ...
  private shouldFilterRequestByUrl(url: string): boolean {
    ...
    return NETWORK_FILTERS.EXCLUDED_DOMAINS.some((pattern) =>
  ...
  private shouldFilterRequestByExtension(
    ...
    try {
      ...
      return NETWORK_FILTERS.STATIC_RESOURCE_EXTENSIONS.some((ext) =>
        path.endsWith(ext),
      );
    } catch {
      return false;
  ...
  private shouldFilterByMimeType(
    ...
    if (
      NETWORK_FILTERS.API_MIME_TYPES.some((apiMime) =>
        mimeType.startsWith(apiMime),
      )
    ) {
      return false;
    }
  ...
  private handleResponseReceived(tabId: number, params: any) {
    ...
    if (
      this.shouldFilterByMimeType(response.mimeType, captureInfo.includeStatic)
    ) {
  ...
  private shouldCaptureResponseBody(requestInfo: NetworkRequestInfo): boolean {
    ...
    if (
      NETWORK_FILTERS.API_MIME_TYPES.some((type) => mimeType.startsWith(type))
    ) {
      return true;
    }
  ...
  private cleanupCapture(tabId: number) {
    ...
    console.log(
      `NetworkDebuggerStartTool: Cleaned up resources for tab ${tabId}.`,
    );
  }
  ...
  async stopCapture(tabId: number, isAutoStop: boolean = false): Promise<any> {
    ...
    if (!captureInfo) {
      return {
        success: false,
        message: 'No capture in progress for this tab.',
      };
    }
    ...
    const allRequests = Object.values(
    ...
    const processedRequests = allRequests.map((req) => {
      const finalReq: Partial<NetworkRequestInfo> &
        Pick<
          NetworkRequestInfo,
          'requestId' | 'url' | 'method' | 'type' | 'status'
        > = { ...req };
    ...
    headerValueCounts.forEach((values, name) => {
      values.forEach((count, value) => {
        if (count === requestsWithHeadersCount) {
          ...
          for (const req of requests) {
            const hdrs = req[headerTypeKey] as
              | Record<string, string>
              | undefined;
            if (hdrs) {
              const foundName = Object.keys(hdrs).find(
    ...
    try {
      if (targetUrl) {
        ...
        if (existingTabs.length > 0 && existingTabs[0]?.id) {
          ...
          await chrome.windows.update(tabToOperateOn.windowId, {
            focused: true,
          });
      ...
      console.error(
        'NetworkDebuggerStartTool: Critical error during execute:',
        error,
      );
...
class NetworkDebuggerStopTool extends BaseBrowserToolExecutor {
    ...
    if (ongoingCaptures.length > 1) {
      ...
      for (const tabId of otherTabIds) {
        try {
          ...
          console.error(
            `NetworkDebuggerStopTool: Error stopping capture on tab ${tabId}:`,
            error,
          );
        }
      }
  ...
  private async performStop(
    ...
    console.log(
      `NetworkDebuggerStopTool: Attempting to stop capture for tab ${tabId}.`,
    );
