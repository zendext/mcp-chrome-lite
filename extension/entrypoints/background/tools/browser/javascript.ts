...
async function executeViaScripting(
  ...
  const innerExecute = async (): Promise<ExecutionResult> => {
    ...
    const result = (firstFrame as { result?: ScriptingExecutionResult })
      ?.result;
    ...
    if (!result || typeof result !== 'object') {
      return {
        ...
        engine: 'scripting',
        error: {
          kind: 'scripting_error',
          message: 'No result returned from executeScript',
        },
      };
    }
    ...
    if (!result.ok) {
      ...
      const isSyntaxError =
        result.error?.name === 'SyntaxError' || /SyntaxError/i.test(rawMessage);
