  ...
  async execute(args: NavigateToolParams): Promise<ToolResult> {
    ...
    try {
      ...
      if (refresh) {
        ...
        const targetTab =
          explicit || (await this.getActiveTabOrThrowInWindow(windowId));
        if (!targetTab.id)
      ...
      const buildUrlPatterns = (input: string): string[] | undefined => {
        ...
        try {
          if (!input.includes('*')) {
            ...
            const hostWithWww = hostNoWww.startsWith('www.')
              ? hostNoWww
              : `www.${hostNoWww}`;
      ...
      const openInNewWindow =
        newWindow || typeof width === 'number' || typeof height === 'number';
      ...
      if (openInNewWindow) {
        ...
        if (targetWindow && targetWindow.id !== undefined) {
          ...
          console.warn(
            'No last focused window found, falling back to creating a new window.',
          );
          ...
          if (fallbackWindow && fallbackWindow.id !== undefined) {
            console.log(
              `URL opened in fallback new Window ID: ${fallbackWindow.id}`,
            );
