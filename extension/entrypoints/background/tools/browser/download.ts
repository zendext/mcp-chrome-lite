...
class HandleDownloadTool extends BaseBrowserToolExecutor {
  ...
  async execute(args: HandleDownloadParams): Promise<ToolResult> {
    ...
    const timeoutMs = Math.max(
      1000,
      Math.min(Number(args?.timeoutMs ?? 60000), 300000),
    );
    ...
    try {
      ...
      return {
        content: [
          {
            type: 'text',
            text: JSON.stringify({ success: true, download: result }),
          },
        ],
        isError: false,
      };
      ...
      return createErrorResponse(
        `Handle download failed: ${e?.message || String(e)}`,
      );
    }
  }
}
...
async function waitForDownload(opts: {
  ...
  return new Promise<any>((resolve, reject) => {
    ...
    const matches = (item: chrome.downloads.DownloadItem) => {
      ...
      return (
        name.includes(filenameContains) ||
        (item.url || '').includes(filenameContains)
      );
    };
    ...
    timer = setTimeout(
      () => onError(new Error('Download wait timed out')),
      timeoutMs,
    );
