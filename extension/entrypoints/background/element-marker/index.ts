        ...
        case BACKGROUND_MESSAGE_TYPES.ELEMENT_MARKER_VALIDATE: {
          ...
          (async () => {
            ...
            if (
              ...
              ensured.center &&
              (typeof req.offsetX === 'number' ||
                typeof req.offsetY === 'number')
            ) {
              const dx = Number.isFinite(req.offsetX as any)
                ? (req.offsetX as number)
                : 0;
              const dy = Number.isFinite(req.offsetY as any)
                ? (req.offsetY as number)
                : 0;
            ...
            try {
              ...
              console.warn(
                '[ElementMarker] Validation failed before tool execution',
                e,
              );
              base.tool = {
                name: 'unknown',
