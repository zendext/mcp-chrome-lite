...
function Popup() {
  ...
  const [endpointInput, setEndpointInput] = createSignal('ws://127.0.0.1:12306/extension');
  const [error, setError] = createSignal('');
  const [notice, setNotice] = createSignal('');

  const refresh = async () => {
    try {
      const response = await chrome.runtime.sendMessage({ type: 'lite_get_status' });
  ...
  const connect = async () => {
    try {
      ...
      const response = await chrome.runtime.sendMessage({ type: 'lite_connect', endpoint });
      if (!response?.success) {
        setStatus((current) => ({ ...current, connected: false }));
        setNotice('');
  ...
  const copyConfig = async () => {
    const config = ['[mcp_servers.chrome-mcp-bridge]', 'command = "mcp-chrome-lite"', ''].join(
      '\n',
    );
    try {
      ...
      setNotice('');
      setError(err instanceof Error ? err.message : 'Failed to copy MCP config.');
    }
  };

  onMount(() => {
    void refresh();
    const listener = (message: { type?: string; connected?: boolean; endpoint?: string }) => {
  ...
  return (
    <main class="lite-popup">
      <header class="lite-header">
        ...
        <span class={status().connected ? 'status status-on' : 'status status-off'}>
