...
describe('lite websocket client', () => {
  ...
  beforeEach(() => {
    ...
    (chrome.runtime.onMessage.addListener as Mock).mockImplementation((nextListener) => {
      listener = nextListener as typeof listener;
    });
  });

  it('does not open websocket during background initialization', async () => {
    const { initLiteWebSocketClient } = await import('@/entrypoints/background/lite/ws-client');
  ...
  it('opens websocket only after explicit connect message', async () => {
    ...
    expect(websocketCtor).toHaveBeenCalledWith('ws://127.0.0.1:12306/extension');
  });
  ...
  it('connects to a user-configured websocket endpoint', async () => {
    ...
    listener?.(
      { type: 'lite_set_endpoint', endpoint: 'ws://localhost:45678/custom-extension' },
      {},
      vi.fn(),
    );
    await Promise.resolve();
    ...
    expect(websocketCtor).toHaveBeenCalledWith('ws://localhost:45678/custom-extension');
  });
  ...
  it('saves and connects with endpoint from explicit connect message', async () => {
    ...
    listener?.({ type: 'lite_connect', endpoint: 'ws://127.0.0.1:56789/extension' }, {}, vi.fn());
    await Promise.resolve();

    expect(chrome.storage.local.set).toHaveBeenCalledWith({
      mcpChromeLiteEndpoint: 'ws://127.0.0.1:56789/extension',
    });
    expect(websocketCtor).toHaveBeenCalledWith('ws://127.0.0.1:56789/extension');
  });
  ...
  it('announces readiness after websocket opens', async () => {
    ...
    expect(websocketSend).toHaveBeenCalledWith(JSON.stringify({ type: 'extension_ready' }));
  });
});
