...
function createHost(hostId: string, zIndex: number): { host: HTMLElement; root: ShadowRoot } {
...
export function createElementPickerController(
  ...
  const render = () => {
    ...
    for (const request of state.requests) {
      ...
      item.className = request.id === state.activeRequestId ? 'item active' : 'item';
      item.tabIndex = 0;
      item.addEventListener('click', () => options.onSetActiveRequest?.(request.id));
